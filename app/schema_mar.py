...
class ImageSchema(Schema):
    ...
    image_format = fields.Str(
        validate=validate.OneOf(["jpg", "jpeg", "png", "tiff", "tif", "webp"])
    )
    ...
    def company_name_validate(cls, data, **kwargs):
        if not data["company_name"].isalpha():
            raise ValidationError(
                "company name must be alphabetic only", "company_name"
            )
    ...
    def image_file_validate(cls, data, **kwargs):
        ...
        if width > 2000:
            raise ValidationError(
                "image width must be less than 2000 pixel", "image_file"
            )
        if height > 2000:
            raise ValidationError(
                "image height must be less than 2000 pixel", "image_file"
            )
