...
from app import utils, models, database, hashing, oauth2
...
async def create_client(
    ...
    client_data = models.Client(username=username, hashed_password=hashed_password, client_id=client_id,
                                client_secret=client_secret)
    db.add(client_data)
    db.commit()
...
def get_client(db: Session = Depends(database.get_db),
               current_user: models.Client = Depends(oauth2.get_current_user),):
